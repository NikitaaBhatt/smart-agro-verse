...
const Register = () => {
  ...
  const [farmerData, setFarmerData] = useState({ name: "", email: "", phone: "", password: "" });
  const [buyerData, setBuyerData] = useState({ name: "", email: "", phone: "", password: "" });

  const handleFarmerRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    try {
      const user = await authAPI.registerFarmer(farmerData);
      login(user);
      toast.success("Registration successful! Welcome to AgroSense!");
  ...
  const handleBuyerRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await authAPI.registerBuyer(buyerData);
      login(user);
      toast.success("Registration successful! Welcome to AgroSense!");
    ...
    <div className="min-h-screen flex items-center justify-center p-4 gradient-earth">
      <Card className="w-full max-w-md shadow-card">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-3xl">Join AgroSense</CardTitle>
          ...
          <Tabs defaultValue={defaultTab}>
            ...
            <TabsContent value="farmer">
              <form onSubmit={handleFarmerRegister} className="space-y-4">
                ...
                <Button type="submit" className="w-full gradient-blockchain shadow-glow" disabled={loading}>
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Register as Farmer"}
                </Button>
              </form>
            </TabsContent>
            ...
            <TabsContent value="buyer">
              <form onSubmit={handleBuyerRegister} className="space-y-4">
                ...
                <Button type="submit" className="w-full gradient-blockchain shadow-glow" disabled={loading}>
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Register as Buyer"}
                </Button>
              </form>
            </TabsContent>
          </Tabs>
